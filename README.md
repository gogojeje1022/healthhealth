...
- 🎨 **테마** — 강조색을 **블랙(기본) / 그린 / 블루 / 핑크** 중 선택할 수 있습니다. 선택은 자동 저장되고 같은 Google 계정으로 로그인한 다른 기기에도 동기화돼요. (구현은 Tailwind `brand-*` 색을 CSS 변수(`--brand-50…900`) 로 바꾸고 `:root[data-theme="..."]` 별로 정의해, 같은 클래스가 테마에 따라 다른 색이 되도록 했습니다.)
